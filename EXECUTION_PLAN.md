...
| `recovery_decisions` | Compliance adjudication only — a closed outcome vocabulary (`executed`, `blocked_cooldown`, `blocked_contact_hours`, `blocked_already_escalated`, `blocked_already_stopped`, `flagged_manual_review`) | `decision_id`, `opportunity_id`, `candidate_id`, `outcome`, `reasoning`, `triggered_by`, `timestamp` |
