...
--------------------------------------------------------------------------
Network health at serving time -- CLOSED in Phase 5
--------------------------------------------------------------------------
This module previously passed bank=None, psp=None, decision_time_hours=0.0,
so every live scoring landed in the network_health_known=0 regime and the
four network-health features were dead. The Phase 4 hand-off filed that as a
Phase 6/7 item on the reading that it depended on data the system did not
have; that reading was wrong, since this project is synthetic end to end and
the gap was always ours to close.

Phase 5 closed it in three parts: `bank`/`psp` columns on payments, a seeded
health series generated by the Data Factory's own generator, and the
unix -> simulated-hour mapping in phase5_config.simulated_hour_for().

The three lines here are a **dated, narrow exception** to this module's frozen
status, granted 2026-09-03 for this purpose only. They are lookup keys, not
model features -- none of `bank`, `psp`, `decision_time_hours` appears in
outcome_features.ALL_FEATURES -- so they reach the model solely through the
four network_health_* features. Nothing about candidate generation, scoring,
EIV arithmetic, ranking or persistence changed. See PHASE5_NOTES.md section
1e for the exception record and 1f for the mapping's measured constraints.
"""
...
from backend.engine.phase5_config import simulated_hour_for
...
def build_optimizer_context(opportunity, latest_payment, customer, merchant,
    ...
    return {
        ...
        # --- network channel, wired in Phase 5 (frozen-list exception) ---
        # These three are lookup keys for the network-health series, not model
        # features: none of them appears in outcome_features.ALL_FEATURES. They
        # reach the model only through the four network_health_* features,
        # which is what made this a narrow plumbing change rather than a
        # scoring change. See PHASE5_NOTES.md section 1e.
        "bank": (latest_payment or {}).get("bank"),
        "psp": (latest_payment or {}).get("psp"),
        "decision_time_hours": simulated_hour_for(now),
    }
