"""
FROZEN LEGACY MODULE -- Phase 2, Data Factory.

This is a byte-for-byte copy of the pre-Phase-2 `backend/ml/simulate_training_data.py`
as it existed at the start of Phase 2, frozen here unmodified.

Why it exists: the two already-shipped model artifacts
(`backend/ml/models/lr_model.joblib`, `xgb_model.joblib`) were trained on a
corpus produced by the ORIGINAL generator. If that generator is ever changed
in place, those artifacts silently become unreproducible from source -- no
one could regenerate the exact training corpus that produced them. Freezing
a copy here means the original artifacts remain reproducible on demand,
without constraining the design of the new, generalized Phase 2 generator
in `backend/data_factory/candidate_outcome_dataset.py` in any way.

DO NOT EDIT THIS FILE. If a bug is ever found in the original generator,
fix it in the live `backend/ml/simulate_training_data.py`, not here -- this
copy's entire value is being an unmodified historical snapshot.

Frozen at: Phase 2 kickoff (see PHASE2_NOTES.md for exact commit context).
Original file: backend/ml/simulate_training_data.py
Original module docstring follows below, unmodified.
"""

# ==== BEGIN VERBATIM ORIGINAL FILE CONTENT (do not edit below this line) ====
"""
